...
class MailPushDevice(models.Model):
    ...
    token = fields.Char(
        string='Access Token', readonly=True, copy=False, index=True,
    ...
    token_expiry = fields.Datetime(
        string='Access Token Expiry', readonly=True, copy=False,
        help="Expiry datetime for the access token. After this the service worker must "
             "use the refresh token to obtain a new access token.")
    refresh_token = fields.Char(
        string='Refresh Token', readonly=True, copy=False,
        groups='base.no_one',
    ...
    refresh_token_expiry = fields.Datetime(
        string='Refresh Token Expiry', readonly=True, copy=False,
        groups='base.no_one')

    _sql_constraints = [
        ('endpoint_unique', 'unique(endpoint)', 'The endpoint must be unique !'),
    ...
    def refresh_subscription_by_token(self, token, endpoint, keys, vapid_public_key,
        ...
        """Update a push subscription endpoint without requiring an active user session.

        Called by the service worker's pushsubscriptionchange handler via
        POST /web/push/device/refresh. The device is looked up by its previous endpoint
        (a non-secret URL) and the access token is verified using secrets.compare_digest()
        to prevent timing side-channels.

        On success the access token is rotated (new 1-hour credential) and the new value
        is returned so the service worker can update its IndexedDB copy.
        ...
        device = self.sudo().search([('endpoint', '=', search_endpoint)], limit=1)
        if not device:
            return {'error': 'device_not_found'}
        # Verify the token with a constant-time comparison to prevent timing attacks.
        if not secrets.compare_digest(device.token or '', token or ''):
            return {'error': 'device_not_found'}
        # Only reveal 'expired' once the token identity has been confirmed.
        if device.token_expiry and device.token_expiry < fields.Datetime.now():
    ...
    def _rotate_tokens(self, refresh_token, previous_endpoint=None):
        ...
        Called by POST /web/push/device/token/rotate when the service worker's access
        token has expired. The refresh token is read from the HttpOnly cookie by the
        controller and passed here.

        The old refresh token is immediately invalidated (one-time use) and a new pair
        is written atomically before the response is sent.
